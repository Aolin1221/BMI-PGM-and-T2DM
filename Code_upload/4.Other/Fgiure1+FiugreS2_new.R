...
prcc<-ggplot(bgsub, aes(x = bmi_calc, y = rank_diff_per)) +
  ...
  scale_x_continuous(limits = c(10,60),breaks = seq(10,60,by=10))+
  geom_hline(yintercept = c(-23.9,24.0),linetype=3,linewidth=0.7,color="gray")+
  theme_bw()+
  theme(
    panel.grid = element_blank(),
    text=element_text(family="serif"),
...
#计算每个BMI组的均值
bmi_group_mean<-aggregate(bgsub$bmi_calc, by=list(type=bgsub$bmi4g),mean)
colnames(bmi_group_mean)<-c("bmi4g", "bmi_mean")
bmi_group_median<-aggregate(bgsub$bmi_calc, by=list(type=bgsub$bmi4g),median)
colnames(bmi_group_median)<-c("bmi4g", "bmi_median")

#合并均值到原始数据框
bgsub<-merge(bgsub,bmi_group_mean,by="bmi4g")
bgsub<-merge(bgsub,bmi_group_median,by="bmi4g")

#绘图
pboxc<-ggplot(bgsub,aes(x=bmi_median, y=rank_diff_per)) +
  geom_boxplot(aes(group=bmi4g),width = 0.4,color=ckb[3],outlier.shape = NA) +
  labs(
    x=expression(paste("BMI (kg/",m^2,")")),
    y="BMI-PGM",
    title="E"
  ...
  scale_x_continuous(limits=c(10,60),breaks=seq(10,60,by=10))+
  scale_y_continuous(limits=c(-100,100),breaks=seq(-100,100,by=25))+
  geom_vline(xintercept=c(18.5,24.0,28.0),linetype=3,linewidth=0.7,color="gray")+
  theme_bw()+
  theme(
    panel.grid=element_blank(),
    text=element_text(family="serif"),
...
####堆积柱状图BMI_CKB####
#生成新的变量重新排列
q<-quantile(bgsub$rank_diff_per)
bgsub$rankqgg<-NA
bgsub$rankqgg[(bgsub$rank_diff_per>=q[2])&(bgsub$rank_diff_per<q[4])]<-2
bgsub$rankqgg[bgsub$rank_diff_per<q[2]]<-3
bgsub$rankqgg[bgsub$rank_diff_per>=q[4]]<-1
bgsub$rankqgg<-factor(bgsub$rankqgg,labels = c("Discordantly high","Concordant","Discordantly low"))

tab_bmi4g<-table(bgsub$bmi4g,bgsub$rankqgg)
...
pbarc<-ggplot(prop_bmi4g, aes(x = bmi4g, y = freq, fill = rankqg)) +
  ...
  labs(title="G",x = "BMI groups", 
       y = "Percentage(%)",fill="BMI-PGM groups") +
  scale_x_continuous(limits = c(10,60),breaks = seq(10,60,by=10))+
  theme_bw()+
  theme(
    panel.grid = element_blank(),
...
pru<-ggplot(datbq, aes( y = bmi,x = SCORE_st)) +
...
round(cor$estimate, 2)#0.07
round(cor$p.value, 2)#0

pru<-pru+annotate("text", x = 4.25, y =15, 
                  label="r = 0.07, p value < 0.001" ,
                  size=5,family="serif",fontface="bold")


####BMI与BMI-PGM相关图_UKB####
pruu<-ggplot(datbq, aes(y = bmi, x = rank_diff_per)) +
  geom_point(colour=ukb[3],size=0.5)+
  geom_smooth(method = "lm", se = FALSE, colour="black",linewidth=0.7) +  
  labs(title="D",x=expression(paste("BMI (kg/",m^2,")")))+
  scale_y_continuous(limits = c(-100,100),breaks = seq(-100,100,by=25))+
  scale_x_continuous(limits = c(10,60),breaks = seq(10,60,by=10))+
  geom_hline(xintercept = c(-24.7,24.6),linetype=3,linewidth=0.7)+
  theme_bw()+
  theme(
    panel.grid = element_blank(),
    text=element_text(family="serif"),
...
round(cor$estimate, 2)#0.32
round(cor$p.value, 2) #0
pruu<-pruu+annotate("text", x = -100, y =60, 
                    label="r = 0.32, p value < 0.001" ,
                    size=5,family="serif",fontface="bold")
...
pbaru<-ggplot(prop_bmi4g, aes(x = bmi4g, y = freq, fill = rankqg)) +
  ...
  geom_text(aes(label = sprintf("%.1f", freq)), 
            position = position_stack(vjust = 0.5), size = 5,family="serif") +
  labs(title="H",x = "BMI groups", fill="BMI-PGM groups") +
  scale_y_continuous(limits = c(-100,100),breaks = seq(-100,100,by=25))+
  scale_x_continuous(limits = c(10,60),breaks = seq(10,60,by=10))+
  theme_bw()+
  theme(
...
cairo_pdf("C:/Users/DELL/Desktop/F2.pdf",width = 15,height = 25)
p9
dev.off()
